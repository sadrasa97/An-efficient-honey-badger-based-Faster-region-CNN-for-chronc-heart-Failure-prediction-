...
def genetic_algorithm_optimize(model, data, labels, population_size=10, generations=10, mutation_rate=0.1):
    def create_population():
        return [tf.keras.models.clone_model(model) for _ in range(population_size)]

    def evaluate_fitness(population):
        scores = []
        for individual in population:
            individual.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
            loss, accuracy = individual.evaluate(data, labels, verbose=0)
            scores.append(accuracy)
        return scores

    def select_parents(fitness_scores):
        parents_indices = np.argsort(fitness_scores)[-2:]  # Select the two best individuals
        return [population[parents_indices[0]], population[parents_indices[1]]]

    def crossover(parent1, parent2):
        child = tf.keras.models.clone_model(model)
        for layer_index in range(len(child.layers)):
            if isinstance(child.layers[layer_index], tf.keras.layers.Dense):
                weights1, biases1 = parent1.layers[layer_index].get_weights()
                weights2, biases2 = parent2.layers[layer_index].get_weights()
                child.layers[layer_index].set_weights([(weights1 + weights2) / 2, (biases1 + biases2) / 2])
        return child
    ...
    def mutate(individual):
        for layer in individual.layers:
            if isinstance(layer, tf.keras.layers.Dense):
                ...
                weights += np.random.normal(0, mutation_rate, weights.shape)
                biases += np.random.normal(0, mutation_rate, biases.shape)
...
optimized_model = genetic_algorithm_optimize(model, train_data, train_labels, population_size=10, generations=10, mutation_rate=0.1)

# Train the model
optimized_model.fit(train_data, train_labels, epochs=120, batch_size=16, verbose=1)
...
def plot_history(history):
    ...
    plt.plot(history.history['val_accuracy'], label='Test Accuracy')  # Add test accuracy to the plot
    ...
    plt.plot(history.history['val_loss'], label='Test Loss')  # Add test loss to the plot
...
history = model.fit(train_data, train_labels, epochs=120, batch_size=16, verbose=1, 
